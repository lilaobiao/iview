<template>
  <dev-article>
    <Row :gutter="16">
      <i-col span="6">
        <Card shadow title="访问量">
          ...
          <Divider></Divider>日访问量 1,230
        </Card>
      </i-col>
      <i-col span="6">
        <Card shadow title="总销售额">
          <Tooltip slot="extra" placement="top">
            <Icon type="ios-alert-outline" size="18"></Icon>
          </Tooltip>
          <div class="count">￥888,000</div>
          <Divider></Divider>周同比12%
          <Icon type="md-arrow-dropup" size="22" color="#ed4014"/>周同比12%
    ...
    <Card shadow style="margin-top:16px">
      <Tabs value="sell">
        <template slot="extra">
          <RadioGroup v-model="dateType" @on-change="handleChange">
            <Radio label="day">今日</Radio>
            <Radio label="week">本周</Radio>
            <Radio label="month">本月</Radio>
            <Radio label="year">全年</Radio>
          </RadioGroup>
          <DatePicker type="daterange" v-model="countDate" style="width:250px"></DatePicker>
        </template>
    ...
    <Modal title="添加快捷操作" v-model="newShortcut.status" @on-ok="handleOk">
